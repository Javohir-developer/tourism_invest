...
//			Yii::$app->session->setFlash('error', __("Email yoki parol noto'g'ri kirtilgan, tekshirib qaytadan kiriting"));
    ...
    public function actionContact()
    {
        ...
        $persons = Person::getReceptions(2);

        return $this->render('contact', ['persons' => $persons]);
    }
	...
	{
	    $query = Post::find()->where(['like', 'content', $input])->orWhere(['like', 'title', $input]);
        ...
        $query = Pages::find()->where(['like', 'body', $input])->orWhere(['like', 'title', $input]);
